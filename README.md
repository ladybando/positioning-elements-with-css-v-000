...
not the intended effect, you can use the `clear` property and set it to clear, or reset,
the element floated `left`, `right`, or elements floating using `both`. An example of
...
In this scenario, we want the footer to cleanly display below all the column. In order to get the
...
the floated content is now falling outside of the parent element. The parent element loses reference to its child elements as the parent element is not floating and its children have broken out of the page flow.
...
The `relative` value for the `position` property allows elements to appear within the normal flow a page. It gives us the ability to offset an element from the top, right, bottom, or left direction, the number of units that are set without changing the positioning of the
elements around it.
...
We will discuss later how to adjust for this limitation.

## Conclusion

CSS enables us to choose from many options with positioning elements. Over the past
couple of years alone, CSS has dramatically changed the way we develop
...
design--whether it's for alignment, spacing, and/orB other design choices.
